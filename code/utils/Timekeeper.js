...
class Time{
     ...
     draw(ctx, startString){
          ...
          var x = ctx.measureText(startString).width

          ctx.fillColor = "white"
          ctx.fillText("H: ", x, 75)
          x += ctx.measureText("H: ").width
          ctx.fillColor = "black"
          ctx.fillText(hourString, x, 75)
          x += ctx.measureText(hourString).width

          ctx.fillColor = "white"
          ctx.fillText(" M: ", x, 75)
          x += ctx.measureText(" M: ").width
          ctx.fillColor = "black"
          ctx.fillText(minString, x, 75)
          x += ctx.measureText(minString).width

          ctx.fillColor = "white"
          ctx.fillText(" S: ", x, 75)
          x += ctx.measureText(" S: ").width
          ctx.fillColor = "black"
          ctx.fillText(secString, x, 75)
          x += ctx.measureText(secString).width
     }
